...
The shipped `headless` profile contains `dsh-base`, `dsh-fork-base`, and `dsh-headless`. The headless bundle supplies its persona and tool mode, disables HMR, mounts the Code Mode worker explicitly, and inserts `headless-runner`. Its tree contains no `@deepseek-ai/dsh-host-*` package, ApiProxy, HTTP server, Web runtime, or browser client. Code Mode and Session persistence are one-shot Agent capabilities independent of Web presentation.
...
`loadProfile` recognizes exact installation-owned headless tuples (`dsh-base`, `dsh-web-app`, `dsh-headless`; and `dsh-base`, `dsh-headless`) and normalizes them to the shipped headless template while preserving every other manifest field. Extra, missing, or reordered bundle lists are user-owned and remain untouched. [Fork-owned Profile bundles](2026-08-25-fork-owned-profile-bundles.md) owns the fork-base layer in that template.
