...
- The escalation fields exist exactly when the mounted executor confines; a request that is not strictly wider than the call's effective mode is treated as omitted and prompts no one; a deployment with no ApprovalService fails strictly-wider escalating calls closed and leaves plain calls untouched.
