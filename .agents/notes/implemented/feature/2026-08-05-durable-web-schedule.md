...
The [`packages/bundle/web-app`](../../../../packages/bundle/web-app/cordis.patch.yml) composition loads `@deepseek-ai/dsh-time-context` and `@deepseek-ai/dsh-schedule` by default. The [`examples/web-schedule`](../../../../examples/web-schedule/README.md) overlay stays as a documented empty `--patch` path so older checkouts and the schedule-after e2e harness still name one file; it must not re-insert those ids. Schedule observes only root Agents published after the plugin loads and installs its tools plus one disposable owner in that Agent scope. Cold history reads, already-published roots, child Agents, and other hosts do not activate it.
