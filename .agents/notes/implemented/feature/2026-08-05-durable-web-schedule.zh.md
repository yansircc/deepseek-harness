...
[`packages/bundle/web-app`](../../../../packages/bundle/web-app/cordis.patch.yml) 组合默认加载 `@deepseek-ai/dsh-time-context` 与 `@deepseek-ai/dsh-schedule`。[`examples/web-schedule`](../../../../examples/web-schedule/README.zh.md) overlay 仍是文档化的空 `--patch` 路径，供旧检出与 schedule-after e2e 使用，且不得再插入这些 id。Schedule 只观察插件加载后发布的根 Agent，并在该 Agent scope 中安装工具和一个可丢弃 owner。cold history 读取、已发布的根、child Agent 与其他 host 都不会激活它。
