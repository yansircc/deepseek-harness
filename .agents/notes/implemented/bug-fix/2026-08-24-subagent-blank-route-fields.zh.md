...
该辅助函数留在各自的包内。`@deepseek-ai/dsh-tool-subagent-control` 不得导入 `@deepseek-ai/dsh-tool-subagent`。面向模型的 schema 描述仍写「省略以继承」／「省略以列出全部路由」；空值按省略处理，是这些工具解释它们已经收到的 JSON 的方式。
...
**在两个包之间共享同一个辅助函数。** 未采纳：函数只有四行，而且控制包导入委派包会反转包依赖方向。
...
`packages/subagent/tool-subagent/tests/route.spec.ts` 固定空白继承、产品传输上的空白字段，以及去空白后的显式 id。`packages/subagent/tool-subagent-control/tests/list-models.spec.ts` 固定空白总览、去空白后的已知 id，以及对未知 id 的拒绝。
