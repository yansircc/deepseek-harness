...
The helper is local to each package. `@deepseek-ai/dsh-tool-list-models` must not import `@deepseek-ai/dsh-tool-subagent`. Blank-as-omit for delegation lives in `@deepseek-ai/dsh-subagent-route-policy`. Model-facing schema descriptions stay "omit to inherit" / "omit to list every route"; empty-as-omit is how those tools interpret the JSON they already receive.
...
`packages/subagent/subagent-route-policy/tests/delegation-route.spec.ts` pins blank inherit, product-transport blanks, and trimmed explicit ids. `packages/llm/tool-list-models/tests/list-models.spec.ts` pins blank overview, trimmed known ids, and unknown-id rejection.
