...
The owning Host halves register schemas for optional `locale.preference` (`zh` or `en`, where absence delegates to the browser), `ui-theme.preference` (`light`, `dark`, or `system`, default `system`), `ui-conversation` (`busyEnter` plus the five stats-line display flags, all default on), and `ui-workspace-git` (the four workspace-git display flags, all default on; see [conversation display toggles](../feature/2026-08-20-conversation-display-toggles.md) and [ui-workspace-git ownership](../architecture/2026-08-25-ui-workspace-git-client-plugin.md)). The local settings provider stores explicit choices in `$DSH_HOME/settings.yaml`, which resolves to `~/.dsh/settings.yaml` under the default home. The API proxy serves every registered namespace to a loopback client; field roles still redact secrets.
