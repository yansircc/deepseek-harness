...
各领域所属的 Host half 注册 schema：可选的 `locale.preference`（`zh` 或 `en`，缺失时交由浏览器决定）、`ui-theme.preference`（`light`、`dark` 或 `system`，默认为 `system`）、`ui-conversation`（`busyEnter` 加上五个统计行展示开关，默认全部为开），以及 `ui-workspace-git`（四个工作区 git 展示开关，默认全部为开；见[会话展示开关](../feature/2026-08-20-conversation-display-toggles.zh.md)与[ui-workspace-git 所有权](../architecture/2026-08-25-ui-workspace-git-client-plugin.zh.md)）。本地 settings 提供方将显式选择存入 `$DSH_HOME/settings.yaml`，在使用默认 home 时，该路径解析为 `~/.dsh/settings.yaml`。API 代理会向回环客户端服务每一个已注册的 namespace；字段角色仍会脱敏机密值。
