...
| `ctx.workspaceGit` | `core` | [`workspace-git`](../packages/workspace/workspace-git) | - | `apiproxy` | - | Samples one cwd for session-header chrome; the sample is never written to a session log. |
