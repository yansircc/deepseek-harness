...
来源：[`packages/llm/tool-list-models/src/index.ts`](../packages/llm/tool-list-models/src/index.ts)

在 `subagent` 与 `subagent_fork` 上进行进程内每次调用路由选择之前使用的实时 LLM 提供方／模型目录。产品级 subagent 传输永不列入。
