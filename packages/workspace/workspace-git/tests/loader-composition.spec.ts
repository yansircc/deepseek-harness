/**
 * REAL-composition proof: the shipped YAML shape boots through the vendored
 * Loader and serves `ctx.workspaceGit.sample`.
 */
...
describe('real Loader composition', () => {
  it('loads the shipped workspace-git YAML shape and publishes the service', async () => {
    root = await mkdtemp(join(tmpdir(), 'dsh-workspace-git-loader-'))
    const configPath = join(root, 'cordis.yml')
    await writeFile(configPath, "- name: '@deepseek-ai/dsh-workspace-git'\n")
    ...
    const modules = new Map<string, unknown>([
      ['@deepseek-ai/dsh-workspace-git', WorkspaceGit],
    ])
