...
Beyond the installed catalog, `src/catalog.ts` carries a small `CATALOG_ADDITIONS` table of models the installed pi-ai has not shipped yet, merged into the route's catalog exactly like shipped entries (`glm-5.3-flash` on `zai-coding-cn` is one). A provider's newest release lands in pi-ai's registry on its own cadence, and the harness serves it from launch day instead of waiting. Each entry is a full catalog model shaped from the route's shipped siblings, and it must be deleted the moment the installed catalog ships the same id — `catalogModels` refuses a collision, so a stale bridge fails loud at load instead of silently shadowing the upstream entry. Entries may only name routes the installed catalog ships; a route pi-ai has never heard of is still declared outright in `models`.
