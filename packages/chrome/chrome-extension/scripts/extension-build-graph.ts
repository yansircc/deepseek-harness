...
export const renderExtensionManifest = (
  ...
  validateChromeExtensionVersion(inputs.extensionVersion)
  ...
  if (
    ...
    throw new Error(
      'Manifest template must not contain build-derived key, version, or minimum_chrome_version',
    )
  }
  ...
  return {
    ...
    version: inputs.extensionVersion,
...
export const validateExtensionDirectory = async (
  ...
  const manifest = asObject(manifestValue, 'Built extension manifest')
  if (manifest.version !== inputs.extensionVersion) {
  ...
  if (
    ...
    evidence.displayVersion !== inputs.extensionVersion ||
