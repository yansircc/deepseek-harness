...
if (
  ...
  throw new Error('Source extension manifest must declare its numeric extension version')
}
const { version: extensionVersion, ...manifestTemplate } = sourceManifest
const fingerprintBuild = await mkdtemp(join(tmpdir(), 'dsh-chrome-fingerprint-'))
...
validateBuildGraph()
const builtManifest = renderExtensionManifest(manifestTemplate, manifestInputs)
...
const prepareExtension = async (directory: string): Promise<void> => {
  for (const [id, bundle] of bundleEntries()) {
    await build({
      ...
      root,
      define: {
        __DSH_CHROME_BRIDGE_URL__: JSON.stringify(bridgeUrl.origin),
        __DSH_CHROME_PROTOCOL_FINGERPRINT__: JSON.stringify(fingerprint),
      },
      plugins: [browserOnly],
      build: {
        emptyOutDir: false,
