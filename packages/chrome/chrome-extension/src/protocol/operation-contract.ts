...
const OPERATION_CONTRACTS = {
  tab: {
    ...
    group: defineOperation(
      ...
      atomicTool(
        ...
        'Group an exact Chrome tab under the DSH session.',
      ),
    ),
  ...
  page: {
    ...
    wait: defineOperation(
      PageCalls.wait,
      opaque('wait observations vary by condition kind'),
      Deadline.wait,
  ...
  system: {
    ...
    'automation-status': defineOperation(
      ...
      atomicTool(
        ...
        'Inspect session automation ownership without changing it.',
      ),
    ),
    'clear-stale': defineOperation(
      SystemCalls['clear-stale'],
      result(ClearStaleResult),
      Deadline.default,
      atomicTool(
...
const PUBLIC_OPERATION_DOMAINS = ['tab', 'page', 'input', 'system'] as const

export const ATOMIC_TOOL_DESCRIPTORS: ReadonlyArray<AtomicToolDescriptor> =
  PUBLIC_OPERATION_DOMAINS.flatMap(domain =>
    Object.entries(OPERATION_CONTRACTS[domain]).flatMap(([operation, contract]) => {
...
export const TabCall = Schema.Union(callsOf(OPERATION_CONTRACTS.tab, 'call')).annotate(
  description(
    'Manage Chrome tabs owned by this DSH session or explicitly selected tabs. Omitted targets require an unambiguous owned set.',
  ),
)
