...
// =================== dsh-chrome automation target ownership ===================
...
const AUTOMATION_TARGETS_STORAGE_KEY = 'dshChromeAutomationTargets'
const BROWSER_EPOCH_STORAGE_KEY = 'dshChromeBrowserEpoch'
...
function sessionKeyOf(params: BrowserTargetParams): string {
  if (typeof params.sessionKey !== 'string' || params.sessionKey.length === 0) {
    throw rejected('missing-session-key', 'Chrome automation requires a DSH session key')
  }
  ...
  for (const [sessionKey, sessionTargets] of Object.entries(targets)) {
    if (sessionTargets.length === 0) {
      throw invalidAutomationTargetState(
        `Chrome automation target storage contains an empty target set for DSH session ${sessionKey}`,
      )
    }
    if (sessionTargets.length > MAX_AUTOMATION_TARGETS_PER_SESSION) {
      throw invalidAutomationTargetState(
        `DSH session ${sessionKey} stores ${sessionTargets.length} automation targets; maximum is ${MAX_AUTOMATION_TARGETS_PER_SESSION}`,
      )
    }
  ...
  const decoded = Object.fromEntries(
    Object.entries(stored).map(([sessionKey, value]) => {
      if (!Array.isArray(value) || value.length === 0) {
        throw invalidAutomationTargetState(
          `Invalid Chrome automation target set for DSH session ${sessionKey}`,
        )
      }
      ...
      if (new Set(identities).size !== identities.length) {
        throw invalidAutomationTargetState(
          `DSH session ${sessionKey} contains duplicate Chrome automation targets`,
        )
      }
...
async function appendAutomationTarget(sessionKey: string, target: AutomationTarget): Promise<void> {
  ...
  if (sessionTargets.length >= MAX_AUTOMATION_TARGETS_PER_SESSION) {
    throw rejected(
      ...
      `DSH session ${sessionKey} already owns ${sessionTargets.length} Chrome automation targets; maximum is ${MAX_AUTOMATION_TARGETS_PER_SESSION}. Close an owned tab before creating another.`,
      {
        scope: 'session',
...
async function regularNormalWindows(): Promise<chrome.windows.Window[]> {
  ...
  return windows.filter(
    window =>
      typeof window.id === 'number' && window.type === 'normal' && ! window.incognito,
  )
}
...
const ownershipLost = (
  ...
  new AutomationOwnershipLost(
    reason === 'epoch-changed'
      ? `DSH session ${sessionKey} has automation ownership from a previous browser epoch. ` +
          'Run /chrome cleanup before creating a replacement; no existing tab was adopted or closed.'
      : reason === 'tab-missing'
        ? `DSH session ${sessionKey} lost its exact automation tab. ` +
...
const ambiguousAutomationTarget = (
  ...
  rejected(
    ...
    `DSH session ${sessionKey} owns ${resolutions.length} Chrome automation targets. Pass one exact target id.`,
...
const AUTO_RECONCILE_STALE_REASONS = new Set<AutomationOwnershipLost['reason']>([
  'epoch-changed',
  'tab-missing',
])

const clearProvedStaleAutomationTargets = async (
  sessionKey: string,
  ...
  for (const resolution of resolutions) {
    ...
    if (reasons !== undefined && !reasons.has(resolution.reason)) continue
    await removeAutomationTarget(sessionKey, resolution.target)
    staleOwnershipsCleared += 1
  }
  return { staleOwnershipsCleared } as const
}

const autoReconcileSafeStaleAutomationTargets = async (sessionKey: string) =>
  clearProvedStaleAutomationTargets(sessionKey, AUTO_RECONCILE_STALE_REASONS)

export async function clearStaleAutomationTargets(sessionKey: string) {
  return withTargetTurn(async () => clearProvedStaleAutomationTargets(sessionKey))
}

export async function clearAllStaleAutomationTargets() {
  return withTargetTurn(async () => {
    const targets = await readAutomationTargets()
    let staleOwnershipsCleared = 0
    for (const sessionKey of Object.keys(targets)) {
      staleOwnershipsCleared += (await clearProvedStaleAutomationTargets(sessionKey))
...
export async function profileStaleAutomationStatus() {
  return withTargetTurn(async () => {
    const targets = await readAutomationTargets()
    const staleTargets: Array<{
      readonly sessionKey: string
      readonly reason: AutomationOwnershipLost['reason']
    ...
    return { staleCount: staleTargets.length, staleTargets } as const
  })
}
...
export async function getTabByParams(
  ...
  if (params.selectedTabId !== undefined) {
    ...
    if (!tab) {
      throw rejected(
        ...
        'No target tab specified and this DSH session has no automation tab yet. ' +
  ...
  // Tabs DSH interacts with (page.* actions) join this session's group so the user can see exactly
  // which tabs DSH is driving. We only adopt *ungrouped* tabs — never hijack a tab the user (or
  // another DSH session) already grouped, since groupTab would otherwise rename that group.
