...
const completeDebuggerAttach = async (
  ...
  try {
    ...
    if (/Another debugger|already attached/i.test(message)) {
      throw new Error(
        `Another debugger is attached to tab ${tabId}; dsh-chrome will not detach or replace it.`,
        { cause: error },
      )
    }
...
export function handleDebuggerDetach(
  ...
  if (reason === 'canceled_by_user') {
    console.warn(
      `[dsh-chrome] debugger canceled by user on tab ${source.tabId}; Chrome input will reattach on next call`,
    )
  }
}
...
function cdpRaw(
  ...
  if (!entry) {
    return Promise.reject(new Error(`dsh-chrome has no debugger ownership record for tab ${tabId}`))
  }
