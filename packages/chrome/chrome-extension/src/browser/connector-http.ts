...
const cancelResponseBody = (body: ReadableStream<Uint8Array>) =>
  Effect.tryPromise({
    ...
    Effect.catch(error =>
      Effect.logWarning('dsh-chrome failed to cancel bridge response body', error.message),
    ),
  )
...
const readResponseText = (
  ...
  return Effect.callback<string, ConnectorHttpFailure>((resume) => {
    ...
    response.text().then(
      (text) => {
        if (text.length > limitBytes) {
          finish(Effect.fail(responseTooLarge(limitBytes)))
          return
        }
    ...
    return Effect.sync(() => {
      ...
      Effect.andThen(cancelResponseBody(body)),
      Effect.catch(cause =>
        Effect.logWarning('dsh-chrome failed to cancel bridge response body', String(cause)),
      ),
    )
  })
}
...
const bridgeRequest = (
  ...
  return Effect.tryPromise({
    try: signal =>
      fetch(`${__DSH_CHROME_BRIDGE_URL__}${route.path}`, {
        ...init,
        method: route.method,
        cache: 'no-store',
      ...
      new ConnectorHttpFailure({
        ...
        message: `Could not reach ${__DSH_CHROME_BRIDGE_URL__}`,
        cause,
      }),
  }).pipe(
    ...
    Effect.timeoutOrElse({
      ...
      orElse: () =>
        Effect.fail(
          new ConnectorHttpFailure({
            code: 'bridge-timeout',
            message: `Timed out after ${timeoutMs}ms waiting for ${__DSH_CHROME_BRIDGE_URL__}${route.path}`,
          }),
        ),
    }),
  )
    ...
    Effect.timeoutOrElse({
      ...
      orElse: () =>
        Effect.fail(
          new ConnectorHttpFailure({
            code: 'bridge-timeout',
            message: `Timed out after ${timeoutMs}ms authenticating ${__DSH_CHROME_BRIDGE_URL__}${BRIDGE_ROUTES[routeName].path}`,
          }),
        ),
    }),
  )
