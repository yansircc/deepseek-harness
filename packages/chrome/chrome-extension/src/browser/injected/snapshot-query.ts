...
export function digestFor(snapshot: PageSnapshot): DshChromeSnapshotDigest {
  return {
    url: snapshot.url,
...
export function diffSnapshot(
  previous: DshChromeSnapshotDigest | null | undefined,
  current: DshChromeSnapshotDigest,
): SnapshotDiff {
  ...
  const prevByUid = new Map<string, DshChromeSnapshotDigestLabel>(
    (previous.labels || []).map(x => [x.uid, x]),
  )
  const curByUid = new Map<string, DshChromeSnapshotDigestLabel>(
    (current.labels || []).map(x => [x.uid, x]),
  )
  const added: Array<DshChromeSnapshotDigestLabel> = []
  const removed: Array<DshChromeSnapshotDigestLabel> = []
  const updated: Array<{
    uid: string
    before: DshChromeSnapshotDigestLabel
    after: DshChromeSnapshotDigestLabel
  }> = []
