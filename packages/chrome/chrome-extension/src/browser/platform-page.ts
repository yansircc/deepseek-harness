...
export async function snapshotInTab(params: SnapshotParams) {
  ...
  const results = await executeScript({
    ...
    func: async (invocationArgs) => {
      try {
        const snapshotPage = (globalThis as typeof globalThis & DshChromeInjectedGlobals).__dshChromeSnapshotPage
        if (typeof snapshotPage !== 'function')
          throw new Error('Snapshot bundle did not install __dshChromeSnapshotPage')
        return {
          ok: true,
          value: await snapshotPage(...(invocationArgs as Parameters<DshChromeSnapshotPage>)),
        }
      } catch (error) {
        return {
          ok: false,
  ...
  const registration = await executeScript({
    ...
    func: (
      issuedActions: ReadonlyArray<{
        readonly id: string
        readonly verbs: ReadonlyArray<DshChromeActionVerb>
      }>,
      ...
      const grant = (globalThis as typeof globalThis & DshChromeInjectedGlobals).__dshChromeGrantActionVerbs
      const markContext = (globalThis as typeof globalThis & DshChromeInjectedGlobals).__dshChromeMarkContextRef
      const register = (globalThis as typeof globalThis & DshChromeInjectedGlobals).__dshChromeRegisterFrontier
      if (
...
export async function readInTab(params: ReadParams): Promise<ReadPageResult> {
  ...
  const results = await executeScript({
    ...
    func: (
      ...
      try {
        const readPage = (globalThis as typeof globalThis & DshChromeInjectedGlobals).__dshChromeReadPage
        if (typeof readPage !== 'function')
          throw new Error('Snapshot bundle did not install __dshChromeReadPage')
...
export async function inspectInTab(params: InspectParams) {
  ...
  const results = await executeScript({
    ...
    func: async (invocationArgs) => {
      try {
        const inspectTarget = (globalThis as typeof globalThis & DshChromeInjectedGlobals).__dshChromeInspectTarget
        if (typeof inspectTarget !== 'function')
          throw new Error('Snapshot bundle did not install __dshChromeInspectTarget')
        return {
          ok: true,
          value: inspectTarget(...(invocationArgs as Parameters<DshChromeInspectTarget>)),
        }
      } catch (error) {
        return {
          ok: false,
