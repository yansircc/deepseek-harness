(function() {
	...
	*/
	...
	/** @internal */
	var ensuring$1 = /*#__PURE__*/ dual(2, (self, finalizer) => onExit(self, (_) => finalizer));
	/** @internal */
	...
	*/
	...
	var staleRecovery = document.querySelector("#stale-recovery");
	var staleStatus = document.querySelector("#stale-status");
	var clearStaleButton = document.querySelector("#clear-stale");
	var effectRuntime = make$7(empty);
	var PopupRecoveryFailure = class extends TaggedError("PopupRecoveryFailure") {};
	var requestAutomationRecovery = (request) => tryPromise({
		try: () => chrome.runtime.sendMessage(request),
		catch: (cause) => new PopupRecoveryFailure({ message: messageOf(cause) })
	}).pipe(flatMap((response) => response.ok ? succeed(response.result) : fail(new PopupRecoveryFailure({ message: response.error }))));
	var refreshStaleStatus = requestAutomationRecovery({ type: "dsh-chrome/automation/stale-status" }).pipe(tap((result) => sync(() => {
	...
	clearStaleButton.addEventListener("click", () => {
		...
		effectRuntime.runCallback(requestAutomationRecovery({ type: "dsh-chrome/automation/clear-stale" }).pipe(tap((result) => sync(() => {
