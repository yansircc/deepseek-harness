(function() {
	...
	*/
	...
	//#endregion
	//#region src/shared/browser-companion.ts
	...
	Struct({ staleOwnershipsCleared: ProfileAutomationTargetCount });
	...
	var OPERATION_CONTRACTS = {
		...
		page: {
			...
			wait: defineOperation(PageCalls.wait, opaque("wait observations vary by condition kind"), Deadline.wait, atomicTool("chrome_wait", "Wait for one typed page condition.", "Wait for a page condition.")),
	...
	//#endregion
	//#region src/protocol/poll-diagnostics.ts
	...
	var recoverPollCommandId = (value) => {
		...
		if (!("type" in value) || value.type !== "command" || !("command" in value)) return void 0;
		const command = value.command;
		if (typeof command !== "object" || command === null || Array.isArray(command)) return void 0;
		return "id" in command && typeof command.id === "string" && command.id.length > 0 ? command.id : void 0;
	};
	var summarizePollBodyForDiagnostic = (value) => {
		...
		if ("call" in command) {
			...
			if (typeof call === "object" && call !== null && !Array.isArray(call) && "op" in call && typeof call.op === "string") commandSummary.call = { op: call.op };
		}
	...
	var collectSecretFreeSchemaIssues = (issue, path = []) => {
		switch (issue._tag) {
			...
			default: return [{
				path,
				message: leafMessage(issue._tag)
			}];
		}
	};
	var fieldPath = (path) => path.length === 0 ? "root" : path.map((segment) => typeof segment === "symbol" ? segment.description ?? "symbol" : String(segment)).join(".");
	var formatPollDecodeDiagnostic = (issues, summary) => {
		...
		return text.length <= 2048 ? text : `${text.slice(0, POLL_DIAGNOSTIC_LIMIT_CHARS - 1)}…`;
	};
	...
	var protocolFailureSchemaIssue = (error) => {
		...
		return typeof cause === "object" && cause !== null && "_tag" in cause ? cause : void 0;
	};
	...
	var readResponseText = (response, limitBytes) => {
		...
		return callback((resume) => {
			...
			response.text().then((text) => {
				...
				finish(fail(new ConnectorHttpFailure({
	...
	var projectProfileConnector = (identity) => succeed({
		...
		protocolFingerprint: "94a7abd5826853efa39ab014dc176f51452ac56a7c2a7be83f6ab659ce1ad816"
	...
	function installDshChromeInstrumentation() {
		...
		return new Promise((resolve, reject) => {
			...
			const timer = setTimeout(() => {
				...
				detachDebugger(tabId).then(() => {
					...
					finish(() => reject(new AggregateError([timeout, resetCause], `Navigation transaction timed out and debugger reset failed for tab ${tabId}`)));
				});
			}, deadlineMs);
			execute().then((value) => {
	...
	var beginDebuggerDetach = (tabId, session) => {
		...
		debuggerStates.set(tabId, transition);
		completeDebuggerDetach(tabId, transition).then(() => {
	...
	function decodeAutomationTarget(target, sessionKey) {
		...
		const ownedValid = (candidate.state === "adopted" || candidate.state === "created") && typeof candidate.tabId === "number" && Number.isInteger(candidate.tabId) && hasExactKeys(candidate, [
			"state",
			"epoch",
			"tabId",
			"label"
		]);
		if (!commonValid || !ownedValid) throw invalidAutomationTargetState(`Invalid Chrome automation target state for DSH session ${sessionKey}`);
		return target;
	}
	var targetIdentity = (target) => `tab:${target.tabId}`;
	...
	async function regularNormalWindows() {
		return (await chrome.windows.getAll({ windowTypes: ["normal"] })).filter((window) => typeof window.id === "number" && window.type === "normal" && !window.incognito);
	}
	...
	var resolveAutomationTargets = async (sessionKey) => {
		...
		return Promise.all(targets.map(async (target) => {
			...
			if (!normalWindowIds.has(tab.windowId) || tab.incognito) return staleResolution(target, "tab-outside-regular-profile");
			return {
				state: "owned",
				target,
				tab
	...
	async function getOrAdoptAutomationTarget(sessionKey, groupTitle) {
		return withTargetTurn(async () => {
			await autoReconcileSafeStaleAutomationTargets(sessionKey);
			const current = await resolveAutomationTargets(sessionKey);
			if (current.length > 1) throw ambiguousAutomationTarget(sessionKey, current);
			...
			const focused = (await regularNormalWindows()).find((window) => window.focused);
			...
			const url = tab?.url ?? "";
			if (!tab || typeof tab.id !== "number" || url.startsWith("chrome://") || url.startsWith("chrome-extension://") || url.startsWith("devtools://")) throw rejected("automation-target-required", "The focused regular Chrome window has no automatable active tab; pass one exact target or run chrome_tab_new");
			await appendAutomationTarget(sessionKey, {
				state: "adopted",
				epoch: await currentBrowserEpoch(),
				tabId: tab.id,
				label: cleanGroupTitle(groupTitle)
			});
			return tab;
		});
	}
	...
	var AUTO_RECONCILE_STALE_REASONS = /* @__PURE__ */ new Set(["epoch-changed", "tab-missing"]);
	...
	async function clearAllStaleAutomationTargets() {
		return withTargetTurn(async () => {
			const targets = await readAutomationTargets();
			let staleOwnershipsCleared = 0;
			for (const sessionKey of Object.keys(targets)) staleOwnershipsCleared += (await clearProvedStaleAutomationTargets(sessionKey)).staleOwnershipsCleared;
			return { staleOwnershipsCleared };
		});
	}
	...
	var planAutomationTargetCleanup = async (onlySessionKey) => {
		...
		const cleanup = [];
		for (const [sessionKey, sessionTargets] of Object.entries(targets)) {
			if (onlySessionKey !== void 0 && sessionKey !== onlySessionKey) continue;
			for (const target of sessionTargets) {
				const tab = target.epoch === epoch ? await chrome.tabs.get(target.tabId).catch(() => null) : null;
				cleanup.push({
					sessionKey,
					target,
					tabId: target.state === "created" && tab ? target.tabId : null,
					stale: tab === null
				});
			}
		}
	...
	async function releaseAutomationTargetTab(tabId) {
		await withTargetTurn(async () => {
			...
			const retained = Object.fromEntries(Object.entries(targets).flatMap(([sessionKey, sessionTargets]) => {
				const entries = sessionTargets.filter((target) => target.epoch !== epoch || target.tabId !== tabId);
...
Current tabs:\n${tabs.filter((candidate) => candidate.id !== void 0).slice(0, 20).map((candidate) => `  ${candidate.id}${candidate.active ? " *" : ""}\t${(candidate.title || "(untitled)").slice(0, 60)}\t${candidate.url || ""}`).join("\n") || "  (none)"}`, {
			...
			tab = createOwnedTarget ? await getOrAdoptAutomationTarget(sessionKey, params.sessionGroupTitle) : await getOwnedAutomationTarget(sessionKey);
		...
		const url = tab.url || "";
		if (url.startsWith("chrome://") || url.startsWith("chrome-extension://") || url.startsWith("devtools://")) throw rejected("protected-tab-url", `Chrome blocks extension automation on protected URL: tab=${tab.id} url=${url}`);
	...
	var resolveAxDomEvidence = async (tabId, node, scopeUid) => {
        ...
        const remember = injectedGlobals.__dshChromeRememberElement;
...
try{return {ok:true,value:await action(...invocationArgs)}}
	...
	*/
	...
	var rejectInvalidPollCommand = (commandId, diagnostic, connector) => postResult(makeWireFailureResult(commandId, new CommandRejected({
		...
		message: diagnostic
	})), connector).pipe(asVoid, tapError((error) => logWarning(`dsh-chrome poll-response-invalid rejection for ${commandId} was not acknowledged`, messageOf(error))));
	var handleInvalidPollBody = (text, error, connector) => gen(function* () {
		...
		const diagnostic = formatPollDecodeDiagnostic(issue === void 0 ? [{
			path: [],
			message: "Invalid poll response"
		}] : collectSecretFreeSchemaIssues(issue), summarizePollBodyForDiagnostic(value));
	...
	var runtime = all([
		...
		}).pipe(tapError((error) => logWarning("dsh-chrome runtime step failed", messageOf(error))), retry({ schedule: sharedBridgeRetrySchedule }), forever),
		currentTimeMillis.pipe(flatMap((now) => tryPromise({
		...
		currentTimeMillis.pipe(flatMap(() => tryPromise({
		...
		})), catch_((error) => logWarning("dsh-chrome keepalive heartbeat failed", error.message)), repeat({ schedule: spaced("20 seconds") }))
	], {
		concurrency: "unbounded",
		discard: true
	});
	...
	var isAutomationRecoveryRequest = (value) => typeof value === "object" && value !== null && "type" in value && (value.type === "dsh-chrome/automation/stale-status" || value.type === "dsh-chrome/automation/clear-stale");
	...
	var handleAutomationRecoveryRequest = (request) => tryPromise({
		try: async () => request.type === "dsh-chrome/automation/stale-status" ? {
			kind: "status",
			value: await profileStaleAutomationStatus()
		} : {
			kind: "cleared",
			value: await clearAllStaleAutomationTargets()
		},
		catch: (cause) => new BrowserRuntimeFailure({
	...
	chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
		...
		if (isConnectorIdentityRequest(message)) {
			launch(handleConnectorIdentityRequest(message).pipe(tap((response) => sync(() => {
				sendResponse(response);
			}))));
			return true;
		}
		if (isAutomationRecoveryRequest(message)) {
			launch(handleAutomationRecoveryRequest(message).pipe(tap((response) => sync(() => {
				sendResponse(response);
			}))));
			return true;
		}
	...
	chrome.runtime.onMessageExternal.addListener((message, _sender, sendResponse) => {
		if (isBrowserCompanionWakeRequest(message)) {
			launch(runtimeOwner.start.pipe(andThen(sync(() => {
				sendResponse({
		...
		const response = handleChromeExtensionProbe(message, chrome.runtime, "94a7abd5826853efa39ab014dc176f51452ac56a7c2a7be83f6ab659ce1ad816");
