(function() {
	...
	function lookupDshChromeElement(uid) {
		const state = getDshChromeState();
	...
	function textOf(element, max) {
		return ((element instanceof HTMLElement ? element.innerText : void 0) || element?.textContent || "").replace(/\s+/g, " ").trim().slice(0, max || 500);
	}
	...
	function layoutSections(elements, forms) {
		...
		const addToSection = (summary, kind) => {
			const source = lookupDshChromeElement(summary.uid);
	...
	function snapshotPage(maxElements, containingText, roleFilter, nearUid, mode, query, maxTextChars, observationRef) {
		...
		if (observationRef) {
			...
			if (frontier) {
				...
				const root = frontier.rootUid ? lookupDshChromeElement(frontier.rootUid) : document;
		...
		let near;
		if (nearUid) near = lookupDshChromeElement(nearUid);
		if (near) {
	...
	function readPage(maxChars, view, query, observationRef) {
		...
		if (observationRef) {
			...
			if (frontier) {
				...
				const frontierRoot = frontier.rootUid ? lookupDshChromeElement(frontier.rootUid) : document;
	...
	function inspectTarget(uid, selector, shouldScrollIntoView) {
		pruneDshChromeElements(getDshChromeState());
		let element = null;
		if (uid) element = lookupDshChromeElement(uid);
	...
	var injectedGlobals = globalThis;
