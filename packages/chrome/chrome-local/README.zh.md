...
Local Service Provider for [`@deepseek-ai/dsh-chrome`](../chrome/README.zh.md). It binds a loopback-only HTTP listener for one authenticated Chrome extension connector and submits model commands directly to its in-process command broker; DSH never calls its own listener as an owner client.
