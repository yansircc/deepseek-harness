...
`AgentOptions` supplies the initial provider/model route, an optional reasoning effort, and an optional positive `maxTokens` output cap. On the first request of a loop instance, create-time `reasoningEffort` and `maxTokens` seed the proposal before `agent/request`; after that instance logs a header, later proposals follow the logged config (plus `agent/request`) rather than re-reading those create-time fields. The concrete loop resolves any exact-model adapter default, records the effective values in the request header, and applies them to each conversation-model request; an explicit Agent option or logged effort wins over adapter defaults, while omission leaves the adapter or provider route default in control.
