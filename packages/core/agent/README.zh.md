...
`AgentOptions` 提供初始的提供方／模型路由、可选的推理（reasoning）强度，以及可选的正数 `maxTokens` 输出上限。在循环实例的首次请求上，创建时的 `reasoningEffort` 与 `maxTokens` 会在 `agent/request` 之前写入提议；该实例记录 header 之后，后续提议遵循已记录的配置（以及 `agent/request`），而不再重读这些创建时字段。具体循环会解析确切模型的适配器默认值，把生效值记录到请求 header，并应用到每次对话模型请求；显式 Agent 选项或已记录的强度优先于适配器默认值，省略时由适配器或提供方路由默认值控制。
