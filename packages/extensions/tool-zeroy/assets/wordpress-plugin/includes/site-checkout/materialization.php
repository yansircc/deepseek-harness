...
function zeroy_checkout_materialization_reference(mixed $value, string $kind, array $refs, array $term_refs, array $media_refs): int|null|WP_Error
{
    ...
    // Checkout documents identify posts as collection/slug; createCanonical
    // binds the slug. Both spellings are one authored identity.
    if ($kind === 'post' && str_contains($value['ref'], '/')) {
        $suffix = substr($value['ref'], strrpos($value['ref'], '/') + 1);
        if (isset($map[$suffix])) return (int) $map[$suffix];
    }
    return zeroy_runtime_error('zeroy_site_commit_ref_missing', "Unknown authored {$kind} ref {$key}.", 409, ['ref' => $key]);
}
