...
本插件让 DSH agent 通过 25 个原子 `chrome_*` 工具加上 `chrome_status`，驱动用户现有的 Chrome（含登录态、cookie 和会话）。本地 Chrome 桥是 DSH 原生实现，没有外部运行时。
...
| `src/protocol/operations.ts` | 25 个工具描述符 | 模型看到的名称与描述 |
...
同一密钥必须与扩展的期望一致。协议指纹默认是随包分发的 v1 指纹，因此捆绑扩展可以直接用。若你用不同协议重建，通过 config 覆盖。
...
- **协议兼容**：指纹是版本标签；在 config 里设置 `protocolFingerprint` 声明本桥所说的扩展协议版本（默认是随包分发的 v1 指纹 `593fb1f9…`）。
