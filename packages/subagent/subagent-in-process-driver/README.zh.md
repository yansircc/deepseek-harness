...
子 agent 会获得父 agent 的工作目录／会话谱系；除非 `request.agentOptions` 覆盖，否则还会通过 `resolveChildAgentOptions`（`subagent/resolve-child-options` waterfall；挂载 `@deepseek-ai/dsh-subagent-route-policy` 时即为最新记录的 `request/header` 中的活跃提供方／模型，否则才是创建时的 options）继承子级 `AgentOptions` 和输出 token 上限。它获得全新的扁平注册作用域：父级所有权不会导入父 agent 的工具限制，也不会建立权限子集。
