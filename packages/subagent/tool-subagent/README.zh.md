...
| `agentOptions` | 传给具体提供方的子 agent `provider`、`model` 和正整数 `maxTokens`；进程内提供方会用显式值覆盖继承的父级选项。进程内工具实例还接受每次调用的 `provider`、`model` 和 `reasoning_effort`；省略的调用字段继承父级当前路由。 |
