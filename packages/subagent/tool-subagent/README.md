...
| `agentOptions` | Provider-specific child `provider`, `model`, and positive `maxTokens`; the in-process provider treats explicit values as overrides of inherited parent options. In-process tool instances also accept per-call `provider`, `model`, and `reasoning_effort`; omitted call fields inherit the parent's active route. |
...
The generated default [`subagent` schema](../../../docs/tool-catalog.md#deepseek-aidsh-tool-subagent) under this instance's configured name while its provider exists. Provider context inheritance changes the tool and prompt descriptions. Enabled background mode adds `run_in_background`: continuable mode documents its `true` default, runtime settlement notice, and explicit foreground override, while one-shot mode documents its `false` default and the job id collected with `job_output` or stopped with `job_kill`. In-process instances — those whose provider honors `AgentOptions` — also expose optional `provider`, `model`, and `reasoning_effort`. Call `list_models` before choosing a different route. A changed provider requires `model`. A changed provider or model drops inherited effort unless the call names one. Product transports omit these fields and reject them at execution. While the tool is visible in an assembly's scope, a `tool:<toolName>` system-prompt section tells the model to start independent continuable delegations together, keep working while they run, and choose foreground only when its next action depends on the result; a tool restriction removes both its schema and this guidance.
...
- **Persona, tool filter, and depth remain instance-fixed** — another persona, tool filter, or depth cap still requires another distinctly named tool. In-process calls may pick `provider`, `model`, and `reasoning_effort`; product transports do not honor those arguments.
